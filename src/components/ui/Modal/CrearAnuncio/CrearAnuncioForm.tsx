...
import type { Anuncio } from "../../../../types/anuncio"; // Ajusta la ruta a tus tipos

interface CrearAnuncioFormProps {
    anuncio?: Anuncio | null; // ◄— Recibe el objeto completo para activar el modo edición
    onAnuncioProcesado?: () => void; // ◄— Callback unificado para refrescar la lista de anuncios
    onCancelar?: () => void;
}
...
export function CrearAnuncioForm({ anuncio, onAnuncioProcesado, onCancelar }: CrearAnuncioFormProps) {
    ...
    const [vistaPreviaUrl, setVistaPreviaUrl] = useState<string | null>(anuncio?.imagen_url || null);
    ...
    const {
        ...
        defaultValues: {
            titulo: anuncio?.titulo || "", // Pre-carga el valor si es edición
    ...
    const removerImagen = (e: React.MouseEvent) => {
        ...
        if (vistaPreviaUrl) {
            // Solo revocamos la URL si es un blob local para evitar fugas de memoria
            if (vistaPreviaUrl.startsWith("blob:")) {
    ...
    const onSubmit = async (data: AnuncioInput) => {
        ...
        try {
            ...
            if (isEditMode && anuncio) {
                // Ejecuta PUT /api/anuncio/:id_anuncio
                await anuncioService.editarAnuncio(anuncio.id_anuncio, formData);
            } else {
                // Ejecuta POST /api/anuncio
